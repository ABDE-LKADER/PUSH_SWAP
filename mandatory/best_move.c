...
/*   Updated: 2024/02/18 15:19:47 by abadouab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_moves(t_stack *stack_a, t_stack *stack_b)
{
	int		top;
	int		moves;
	t_stack	*loop;
	...
	moves = 0;
	loop = stack_a;
	top = set_top(stack_a, stack_b->num);
	while (loop->num != top)
	{
		moves++;
		loop = loop->next;
...
static int	best_val(t_stack *stack_a, t_stack *stack_b)
{
	int		size;
	int		moves;
	int		index;
...
void	best_move(t_stack **stack_a, t_stack **stack_b)
{
	...
	bestVal = best_val(*stack_a, *stack_b);
